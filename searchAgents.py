...
def cornersHeuristic(state, problem):
    ...
    temp = node
    while unvisited:
        distance, Corner = min([(util.manhattanDistance(temp, corner), corner) for corner in unvisited])
        cost = cost + distance
        temp = Corner
        unvisited.remove(Corner)
...
def foodHeuristic(state, problem):
    ...
    for Food in foodGrid.asList():
        ...
        while unvisited:
            distance, foods = min([(util.manhattanDistance(temp, food), food) for food in unvisited])
            cost = cost + distance
            temp = foods
            unvisited.remove(foods)
        listOfCost.append(cost)
    return min(listOfCost)
